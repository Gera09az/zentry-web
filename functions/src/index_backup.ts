...
import {onRequest} from "firebase-functions/v2/https"; // Para las funciones HTTP que permanecen Gen2
import {onCall, HttpsError} from "firebase-functions/v2/https"; // Para funciones Callable Gen2
import * as logger from "firebase-functions/logger"; // Logger de Gen2
 ...
 */
export const setUserGlobalAdminClaim = onRequest(
  async (request, response) => {
    ...
    try {
      ...
      if (decodedToken.uid !== SUPER_ADMIN_UID) {
        logger.error(
          `setUserGlobalAdminClaim: Forbidden - Caller ${decodedToken.uid} is not the super admin.`
        );
    ...
    try {
      ...
      logger.info(
        `Successfully set isGlobalAdmin=${isGlobalAdmin} for user ${uid} by super admin ${SUPER_ADMIN_UID}`
      );
      ...
      logger.error(
        `Error setting custom claim for user ${uid}:`,
        error
      );
 ...
 * (Esta función permanece como onRequest, pero usando el import de v2)
 */
export const setResidencialAdminStatus = onRequest(
  async (request, response) => {
    // Permitir CORS
    response.set('Access-Control-Allow-Origin', '*');
    response.set('Access-Control-Allow-Methods', 'POST, OPTIONS');
    ...
    let callingUid: string;
    try {
      const decodedToken = await admin.auth().verifyIdToken(idToken);
      if (decodedToken.uid !== SUPER_ADMIN_UID) {
        logger.error(
          `setResidencialAdminStatus: Forbidden - Caller ${decodedToken.uid} is not the super admin.`
        );
        response.status(403).send("Forbidden: Caller is not authorized.");
        return;
      }
    ...
    try {
      let claimsToSet: { [key: string]: any };
      if (isAdmin) {
        claimsToSet = { 
      ...
      logger.info(
        `Successfully set residencial admin status for user ${targetUid} by super admin ${callingUid}. Claims:`, claimsToSet
      );
      response.status(200).send({
      ...
      logger.error(
        `Error setting residencial admin status for user ${targetUid}:`,
        error
      );
      response.status(500).send({ error: "Internal Server Error", details: error });
    }
 ...
 */
export const crearCuentaStripeResidencial = onRequest( // Temporalmente onRequest v2
  async (request, response) => {
    // Permitir CORS
    response.set('Access-Control-Allow-Origin', '*');
    response.set('Access-Control-Allow-Methods', 'POST, OPTIONS');
    ...
    try {
      ...
      if (residencialData?.stripeAccountId) {
...
export const stripeWebhookHandler = onRequest(
  async (request, response) => {
    ...
    } catch (err: any) { // Asegurar que err es de tipo any o Error
      logger.error(`stripeWebhookHandler: Webhook signature verification failed: ${err.message}`);
      response.status(400).send(`Webhook Error: ${err.message}`);
      return;
    }

    logger.info(`stripeWebhookHandler: Received event ${event.type} for account ${event.account || (event.data.object as any).id}`);
 ...
 */
export const obtenerEstadoCuentaStripe = onCall(
  { region: "us-central1" },
  async (request) => {
    ...
    if (!residencialId) {
      throw new HttpsError("invalid-argument", "El 'residencialId' es requerido.");
    }
    ...
    try {
      ...
      if (!residencialDoc.exists) {
        throw new HttpsError("not-found", "El residencial no fue encontrado.");
      }

      const stripeAccountId = residencialDoc.data()?.stripeAccountId;
      ...
      if (!stripe) {
        throw new HttpsError("internal", "El servicio de Stripe no está inicializado.");
      }
      ...
      if (error instanceof Stripe.errors.StripeError) {
        throw new HttpsError("internal", `Error de Stripe: ${error.message}`);
      }
      throw new HttpsError("internal", "Ocurrió un error inesperado al consultar la cuenta de Stripe.");
    }
  }
);
...
export const createSecurityUserWeb = onCall(async (request) => {
...
export const createtestusers = onRequest(async (req, res) => {
    logger.warn("Función 'createtestusers' llamada pero no implementada.");
    res.status(501).send("Not Implemented");
});

export const debugusers = onRequest(async (req, res) => {
    logger.warn("Función 'debugusers' llamada pero no implementada.");
    res.status(501).send("Not Implemented");
});

export const forcesetadminrole = onRequest(async (req, res) => {
    logger.warn("Función 'forcesetadminrole' llamada pero no implementada.");
    res.status(501).send("Not Implemented");
});

export const forcesetrole = onRequest(async (req, res) => {
    logger.warn("Función 'forcesetrole' llamada pero no implementada.");
    res.status(501).send("Not Implemented");
});

export const setresidencialadminstatus = onCall(async (request) => {
...
export const setsecurityrole = onRequest(async (req, res) => {
...
export const stripewebhookhandler = onCall(async (request) => {
    logger.warn("Función 'stripewebhookhandler' llamada pero no implementada.");
    throw new HttpsError("unimplemented", "Function not implemented.");
});

export const registerpayment = onRequest(async (req, res) => {
    logger.warn("Función 'registerpayment' llamada pero no implementada.");
    res.status(501).send("Not Implemented");
});

export const verifypaymentstatus = onRequest(async (req, res) => {
