...
const RevisarDocumentosDialog: React.FC<RevisarDocumentosDialogProps> = ({ usuario, onClose, todosLosUsuarios, getResidencialNombre }) => {
  ...
  const handleRechazar = async () => {
    ...
    try {
      await actualizarUsuario(usuario.id, { status: 'rejected', rejectionReason: motivo });
      toast.dismiss(loadingToast);
  ...
  return (
    <DialogContent className="w-full max-w-full md:max-w-5xl max-h-[90vh] overflow-y-auto p-0">
      ...
      <div className="w-full flex flex-col md:flex-row gap-4 justify-center items-center mt-6 mb-2 px-3">
        <Button variant="destructive" size="lg" className="flex-1 md:flex-none md:w-48" onClick={() => setShowRechazar(true)}>Rechazar solicitud</Button>
        <Button variant="default" size="lg" className="flex-1 md:flex-none md:w-48 bg-green-600 hover:bg-green-700 text-white" onClick={() => setShowAprobar(true)}>Aprobar solicitud</Button>
      </div>
      ...
      <ConfirmDialog open={showAprobar} onOpenChange={setShowAprobar}>
        <ConfirmDialogContent>
          ...
          <ConfirmDialogFooter>
            ...
            <Button variant="default" className="bg-green-600 hover:bg-green-700 text-white" onClick={handleAprobar}>Confirmar aprobación</Button>
          </ConfirmDialogFooter>
        </ConfirmDialogContent>
      </ConfirmDialog>
      ...
      <ConfirmDialog open={showRechazar} onOpenChange={setShowRechazar}>
        <ConfirmDialogContent>
          ...
          <ConfirmDialogFooter>
            ...
            <Button variant="destructive" disabled={motivoSeleccionado === '' || (motivoSeleccionado === 'Otro' && motivoOtro.trim() === '')} onClick={handleRechazar}>Confirmar rechazo</Button>
          </ConfirmDialogFooter>
        </ConfirmDialogContent>
      </ConfirmDialog>
    </DialogContent>
