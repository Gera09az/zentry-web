...
export const AdminService = {
  ...
  subscribeToSystemStats: (onUpdate: (stats: any) => void) => {
    ...
    const unsubUsers = onSnapshot(usersRef, snap => {
      console.log(`[AdminService] Actualización de usuarios totales: ${snap.size}`);
      updateStats('totalUsers', snap.size);
    });
    ...
    const unsubActiveUsers = onSnapshot(activeUsersQuery, snap => {
      console.log(`[AdminService] Actualización de usuarios activos: ${snap.size}`);
      updateStats('activeUsers', snap.size);
    });
